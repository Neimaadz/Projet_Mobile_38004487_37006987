...
//  UnivMap
//
//  Created by Dylan Cherrier on 07/04/2021.
//
...
class ThemeController: UIViewController {
    ...
    func update(){
        view.backgroundColor = defaults.color(key: COLOR_BACKGROUND)
        scrollView.backgroundColor = defaults.color(key: COLOR_BACKGROUND)
        colorBackground.tintColor = defaults.color(key: COLOR_BACKGROUND)
        colorBackground.layer.borderColor = defaults.color(key: COLOR_BACKGROUND)?.cgColor
    }
...
extension ThemeController : UIColorPickerViewControllerDelegate{
